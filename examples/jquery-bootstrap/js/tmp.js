...
var seriesDefault = [{ id: 1, stroke: 'black', strokeWidth: 2, theeter: 1, margin: 4 / Math.PI, checked: true, title: '4Sin(x)/PI' }, { id: 2, stroke: 'red', strokeWidth: 2, theeter: 3, margin: 4 / (3 * Math.PI), checked: true, title: '4Sin(x)/3PI' }, { id: 3, stroke: 'blue', strokeWidth: 2, theeter: 5, margin: 4 / (5 * Math.PI), checked: true, title: '4Sin(x)/5PI' }, { id: 4, stroke: 'darkgoldenrod', strokeWidth: 2, theeter: 7, margin: 4 / (7 * Math.PI), checked: true, title: '4Sin(x)/7PI' }, { id: 5, stroke: 'darkred', strokeWidth: 2, theeter: 9, margin: 4 / (9 * Math.PI), checked: true, title: '4Sin(x)/9PI' }, { id: 6, stroke: '#666', strokeWidth: 2, theeter: 11, margin: 4 / (11 * Math.PI), checked: true, title: '4Sin(x)/11PI' }];
...
var MyFuliye = React.createClass({
    ...
    getSinPoints: function (item) {
        ...
        for (var i = 0; i < maxI; i++) {
            points = points + i + ',' + (-1 * yOfOne * item.margin * Math.sin(item.theeter * 2 * Math.PI * i / roundTripX) + panning).toFixed(0) + ' ';
    ...
    sum: function () {
        ...
        return React.createElement(
            ...
            React.createElement(
                'label',
                { className: 'seriesTitle', style: { position: 'relative', top: -(totalHeight / 2 - 3) + 'px', fontWeight: 'bold', color: config.sumStroke } },
                'Combined:'
            ),
            React.createElement(
                'svg',
                { width: config.totalWidth, height: totalHeight },
                React.createElement('polyline', { points: points, style: { fill: 'none', stroke: config.sumStroke, strokeWidth: config.sumStrokeWidth } }),
                React.createElement('polyline', { points: x, style: { fill: 'none', stroke: 'black', strokeWidth: 1 } }),
                React.createElement('polyline', { points: y, style: { fill: 'none', stroke: 'black', strokeWidth: 1 } })
            )
        );
    },
    ...
    render: function () {
        ...
        var controlArea = React.createElement(
            'div',
            ...
            React.createElement(
                ...
                ', and restarted again when finished the painting): '
            ),
            React.createElement(
                'span',
                { className: 'showTime' },
            ...
            React.createElement(
                'div',
                { className: 'buttons' },
                React.createElement('input', { type: 'button', value: 'Stop', className: 'stop' }),
                React.createElement('input', { type: 'button', value: 'Resume', className: 'resume' }),
                React.createElement('input', { type: 'button', value: 'Reset', className: 'reset' }),
                React.createElement('input', { type: 'button', value: 'Add', className: 'add' }),
                React.createElement('input', { type: 'button', value: 'Show/Hide details', className: 'showOrHide' })
            )
        );
        ...
        var series = this.props.settings.series.map(function (item, i) {
            ...
            return React.createElement(
                'div',
                { key: item.id, className: 'aSeries' },
                React.createElement(
                    'label',
                    { className: 'seriesTitle', style: { position: 'relative', top: -(totalHeight / 2 - 15) + 'px', fontWeight: 'bold', color: item.stroke } },
                    React.createElement(
                        'div',
                        null,
                        item.title,
                        ':',
                        React.createElement('input', { type: 'checkbox', checked: item.checked, onChange: this.onFilter.bind(null, item) })
                    ),
                    React.createElement(
                        'div',
                        null,
                        'x:',
                        React.createElement('input', { type: 'text', value: item.theeter, style: { width: '18px' }, onChange: this.onXChange.bind(null, item) }),
                        ' ',
                        'margin:',
                        React.createElement('input', { type: 'text', value: item.margin, style: { width: '40px' }, onChange: this.onMarginChange.bind(null, item) })
                    )
                ),
                React.createElement(
                    'svg',
                    { width: this.props.settings.config.totalWidth, height: totalHeight },
                    React.createElement('polyline', { points: points, style: { fill: 'none', stroke: item.stroke, strokeWidth: item.strokeWidth } }),
                    React.createElement('polyline', { points: x, style: { fill: 'none', stroke: 'black', strokeWidth: 1 } }),
                    React.createElement('polyline', { points: y, style: { fill: 'none', stroke: 'black', strokeWidth: 1 } })
                )
            );
...
ReactDOM.render(React.createElement(MyFuliye, { settings: data }), $('#container')[0]);
